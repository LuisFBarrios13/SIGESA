...
import { ResumenPeriodo } from '../notas/resumen_periodo.model.js';

// ── Asociaciones Usuario ──────────────────────────────────────
Usuario.belongsToMany(Rol, {
  through: UsuarioRol, foreignKey: 'id_usuario', otherKey: 'id_rol', as: 'roles',
});
Rol.belongsToMany(Usuario, {
  through: UsuarioRol, foreignKey: 'id_rol', otherKey: 'id_usuario', as: 'usuarios',
});
...
Estudiante.belongsToMany(Acudiente, {
  through: RelacionEA, foreignKey: 'id_estudiante', otherKey: 'id_acudiente', as: 'acudientes',
});
Acudiente.belongsToMany(Estudiante, {
  through: RelacionEA, foreignKey: 'id_acudiente', otherKey: 'id_estudiante', as: 'estudiantes',
});
...
// ── Asociaciones ResumenPeriodo ───────────────────────────────
ResumenPeriodo.belongsTo(Matricula, { foreignKey: 'id_matricula', as: 'matricula' });
ResumenPeriodo.belongsTo(Periodo,   { foreignKey: 'id_periodo',   as: 'periodo' });
Matricula.hasMany(ResumenPeriodo,   { foreignKey: 'id_matricula', as: 'resumenes' });

// ── Exportar todo ─────────────────────────────────────────────
export {
  Rol, Usuario, UsuarioRol,
  Acudiente, Docente,
  Estudiante, RelacionEA,
  Matricula, Grado, Periodo,
  ConceptoPago, MetodoPago, CuentaCobro, Pago, Tarifa,
  Materia, Nota, ResumenPeriodo,
};